...
#     negative_similarity = torch.einsum('nc,ck->nk', student_anchor, teacher_negative)
...
def compute_infoNCE_loss(student_anchor, teacher_positive, teacher_negative_list, temperature=1.0):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # 计算positive pair的相似度
    positive_similarity = torch.einsum('nc,nc->n', student_anchor, teacher_positive)

    # 计算negative pair的相似度，这里假设teacher_negative_list是一个包含负样本logits的列表
    negative_similarity_list = [torch.einsum('nc,nc->n', student_anchor, neg) for neg in teacher_negative_list]

    # 将所有负样本的相似度合并到一个张量中
    negative_similarity = torch.stack(negative_similarity_list, dim=1)

    # 将positive和negative的相似度合并成logits
    logits = torch.cat([positive_similarity.unsqueeze(-1), negative_similarity], dim=1).to(device)

    # 构造对应的labels，这里简化为0表示positive，1表示negative
    
    targets = torch.zeros(logits.size(0), dtype=torch.long).to(device)
...
def train_student_model(teacher_model, student_model, train_dataloader, test_dataloader, num_epochs=10, lr=0.001, step_lr=3, temperature=3.0):
    ...
    log_dir = f"./{model_name}_{dataset_name}_KD_contrastive_tri_mult/{model_name}_{dataset_name}_{time_str}_KD_log"
    ...
    for epoch in range(num_epochs):
        ...
        for batch_index, batch in enumerate(train_dataloader):   
            ...
            negative_samples = [neg.to(device) for neg in negatives]
            ...
            teacher_logits_negatives = [teacher_model(neg) for neg in negative_samples]  # 一组负样本的 logits

            student_logits_anchor = student_model(anchor_samples)
            student_logits_positive = student_model(positive_samples)
            student_logits_negatives = [student_model(neg) for neg in negative_samples]  # 一组负样本的 logits

            # print(student_logits_anchor.shape)32,101

            #InfoNCE loss
            InfoNCE_loss = compute_infoNCE_loss(student_logits_anchor, teacher_logits_positive, teacher_logits_negatives, temperature=1.0)
            # print(InfoNCE_loss)
            #kd_loss
        ...
        if test_top1_acc > best_accuracy:
            ...
            best_epoch = epoch
            torch.save(student_model.state_dict(), f'./{model_name}_{dataset_name}_KD_contrastive_tri_mult/{model_name}_{epoch}_pretrained_KD.pth')    
...
if __name__ == "__main__":
    ...
    pathlib.Path(f"./{model_name}_{dataset_name}_KD_contrastive_tri_mult").mkdir(parents=True, exist_ok=True)
    print('dataset preparing')
    train_food101, test_food101 = dataset_tri_mult.food101()
