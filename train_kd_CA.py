...
def train_student_model(teacher_model, student_model, train_dataloader, test_dataloader, num_epochs=10, lr=0.001, step_lr=3, temperature=3.0):
    ...
    log_dir = f"./{model_name}_{dataset_name}_KD_CA/{model_name}_{dataset_name}_{time_str}_KD_log"
    ...
    for epoch in range(num_epochs):
        ...
        if test_top1_acc > best_accuracy:
            ...
            best_epoch = epoch
            torch.save(student_model.state_dict(), f'./{model_name}_{dataset_name}_KD_CA/{model_name}_{epoch}_pretrained_KD.pth')
        scheduler.step()
...
if __name__ == "__main__":
    ...
    pathlib.Path(f"./{model_name}_{dataset_name}_KD_CA").mkdir(parents=True, exist_ok=True)
