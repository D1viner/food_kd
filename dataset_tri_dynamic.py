...
class Food101TripletDataset(Dataset):
    ...
    def __len__(self):
        return len(self.food101_dataset)

    def __getitem__(self, anchor_idx):
        anchor_image, anchor_label = self.food101_dataset[anchor_idx]

        positive_idx, negative_idx = self._generate_triplet_indices(anchor_label)

        positive_image,positive_label = self.food101_dataset[positive_idx]
        negative_image,negative_label = self.food101_dataset[negative_idx]
    ...
    def _generate_triplet_indices(self, anchor_label):
        label_set = list(self.labels_to_indices.keys())
        
        # 使用集合运算确保选择的负样本与锚点样本的标签不相同
        negative_label_set = set(label_set) - set([anchor_label])
        negative_label = random.choice(list(negative_label_set))
