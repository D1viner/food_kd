...
def compute_infoNCE_loss(teacher_anchor, student_positive, student_negative, temperature=1.0):
    device = teacher_anchor.device  # 获取当前张量所在的设备

    batch_size = teacher_anchor.size(0)

    positive_similarity = torch.einsum('nc,nc->n', teacher_anchor, student_positive)

    negative_similarity = torch.einsum('nc,nc->n', teacher_anchor, student_negative)
...
# def triplet_loss_fixed_margin(anchor, positive, negative, fixed_margin=0.2):
#     distance_positive = nn.functional.pairwise_distance(anchor, positive)
#     distance_negative = nn.functional.pairwise_distance(anchor, negative)
    
#     loss = nn.functional.relu(distance_positive - distance_negative + fixed_margin)
...
def train_student_model(teacher_model, student_model, train_dataloader, test_dataloader, num_epochs=10, lr=0.001, step_lr=3, temperature=3.0):
    ...
    log_dir = f"./{model_name}_{dataset_name}_KD_contrastive_tri_triplet_beta=0.4_T=3/{model_name}_{dataset_name}_{time_str}_KD_log"
    ...
    for epoch in range(num_epochs):
        ...
        for batch_index, batch in enumerate(train_dataloader):   
            ...
            # cross_entropy_loss_2 = criterion(student_logits_positive, labels)
        ...
        if test_top1_acc > best_accuracy:
            ...
            best_epoch = epoch
            torch.save(student_model.state_dict(), f'./{model_name}_{dataset_name}_KD_contrastive_tri_triplet_beta=0.4_T=3/{model_name}_{epoch}_pretrained_KD.pth')    
...
if __name__ == "__main__":
    ...
    pathlib.Path(f"./{model_name}_{dataset_name}_KD_contrastive_tri_triplet_beta=0.4_T=3").mkdir(parents=True, exist_ok=True)
