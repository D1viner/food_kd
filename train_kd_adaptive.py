...
def train_student_model(teacher_model, student_model, train_dataloader, test_dataloader, num_epochs=10, lr=0.001, step_lr=3, temperature=3.0):
    ...
    log_dir = f"./{model_name}_{dataset_name}_KD_adaptive_test_batch_size/{model_name}_{dataset_name}_{time_str}_KD_log"
    ...
    for epoch in range(num_epochs):
        ...
        total_samples = 0
        correct = 0
        batch_counter = 0  # Initialize a counter for the batch size
        for batch_index, (inputs, labels) in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                ...
                hard_labels_loss = criterion(teacher_logits, labels)

                if batch_counter % 32 == 0:
                    D = torch.empty([1]).to(device)
                D = torch.cat((D,hard_labels_loss.unsqueeze(0)))
                batch_counter += 1
                # print(D)
            if batch_index>2:
                normalized_distance = (hard_labels_loss - torch.min(D)) / (torch.max(D) - torch.min(D))
            ...
            # print(torch.min(D)) 
            # print(torch.max(D))
            adaptive_weight = 1 - normalized_distance

            adaptive_loss = criterion(student_logits, labels)

            distillation_loss = criterion_KLDiv(torch.nn.functional.log_softmax(student_logits / temperature, dim=1), soft_labels)

            loss = adaptive_loss*adaptive_weight + distillation_loss
        ...
        if test_top1_acc > best_accuracy:
            ...
            best_epoch = epoch
            torch.save(student_model.state_dict(), f'./{model_name}_{dataset_name}_KD_adaptive_test_batch_size/{model_name}_{epoch}_pretrained_KD.pth')
        scheduler.step()
...
if __name__ == "__main__":
    ...
    pathlib.Path(f"./{model_name}_{dataset_name}_KD_adaptive_test_batch_size").mkdir(parents=True, exist_ok=True)
